...
# python src/python/clf_referee.py  out/p41/d1786/en.drs.clf -s src/python/clf_signature.yaml  -v 5
...
def check_clf(clf, signature, v=0):
    ...
    (op_types, ops_fine, arg_typing) = clf_typing(clf, signature, v=v)
    ...
    # TODO This is partially implemented. Not a crucial component
    # find boxes that do not subordinate anything
    # non_sub_boxes = non_subordinating_boxes(box_dict, sub_rel, v=v)
    # if len(non_sub_boxes) != 1:
    #    raise RuntimeError('More than one non-subordinating boxes || {}'.format(pr_set(non_sub_boxes)))

    return (box_dict, sub_rel, direct_sub, disc_rels, op_types, ops_fine)
...
def clf_typing(clf, signature, v=0):
    ...
    arg_typing = dict()
    op_types = []
    ops_fine = [] # fine-grained info about operators
    # check each clause individually and get variable types
    for cl in clf:
        (op_type, op_fine, typing) = clause_typing(cl, signature, v=v)
        op_types.append(op_type)
        ops_fine.append(op_fine)
    ...
    return (op_types, ops_fine, arg_typing)
...
def clause_typing(clause, signature, v=0):
    ...
    op_fine = operator if op_type in ['DRL'] else op_type
    # check if typing per se is ok
    return (op_type, op_fine, typing)
...
def get_signature(sig_file, out='{op:(kind,types)}', v=0):
    ...
    if os.path.isfile(sig_file):
        with codecs.open(sig_file, 'r', encoding='UTF-8') as f:
            sig = yaml.load(f, Loader=yaml.FullLoader)
    else:
...
def pr_box(box, indent=0):
    ...
    #    for r in box.rels:
    #        print ind, '[ ', ' '.join(r), ' ]'
    if box.cond_refs: print(ind, '{ ', ' '.join(box.cond_refs), ' }')
    #if box.rel_boxes: print ind, '[{ ', ' '.join(box.rel_boxes), ' ]]'
    if box.subs: print(ind, '< ', ' '.join(box.subs), ' >')
